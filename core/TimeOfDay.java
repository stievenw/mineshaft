...
public class TimeOfDay {
    ...
    /**
     * ✅ UPDATED: New brightness calculation (brighter night!)
     */
    private float calculateBrightness(long time) {
        if (time >= 0 && time < 12000) {
            ...
            // Sunset transition
            float t = (time - 12000) / 1000.0f;
            return 1.0f - (t * 0.4f); // ✅ CHANGED: 0.7f → 0.4f (brighter sunset, goes to 0.6)
        } else if (time >= 13000 && time < 23000) {
            // Night
            return 0.6f; // ✅ CHANGED: 0.3f → 0.6f (much brighter night!)
        } else {
            // Sunrise transition
            float t = (time - 23000) / 1000.0f;
            return 0.6f + (t * 0.4f); // ✅ CHANGED: 0.3f+0.7f → 0.6f+0.4f
        }
    }
