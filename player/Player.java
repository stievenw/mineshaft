...
public class Player extends Entity {
    ...
    private static final float WATER_JUMP_BOOST = 0.35f;
    private static final float TERMINAL_VELOCITY = -3.92f;
    private static final float WATER_TERMINAL_VELOCITY = -0.08f;
    private static final float MAX_WATER_UP_SPEED = 0.30f;

    // Movement constants
    private static final float STEP_HEIGHT = 0.6f;
    private static final float GROUND_SNAP_DISTANCE = 0.02f;
    ...
    private void processMovement(float delta) {
        ...
        if (!flying) {
            sneaking = shiftPressed;
        } else {
            sneaking = false; // Cannot sneak while flying
        }
        ...
        if (flying) {
            handleFlyingVerticalMovement(speed);
        } else {
            handleJumping();
        }
    }
    ...
    private void handleJumping() {
        ...
        if (inWater) {
            ...
            if (onGround && !sneaking) { // Cannot jump while sneaking
    ...
    private void moveWithCollisionAndStep(float dx, float dz) {
        ...
        if (canStep) {
            ...
            for (float stepHeight : commonSteps) {
                if (stepHeight > STEP_HEIGHT)
                    continue;
            ...
            for (float stepHeight = 0.05f; stepHeight <= STEP_HEIGHT; stepHeight += 0.05f) {
                float testY = y + stepHeight;
    ...
    private void applyPhysics(float delta) {
        ...
        if (onGround && velocityY == 0) {
            ...
            if (groundY > 0) {
                float distToGround = y - groundY;

                if (Math.abs(distToGround) < 0.001f) {
                    y = groundY;
                    return;
                } else if (distToGround > 0 && distToGround < GROUND_SNAP_DISTANCE) {
                    y = groundY;
                    return;
                } else if (distToGround < -0.1f) {
                    y = groundY;
                    velocityY = 0;
                    return;
                } else if (distToGround > GROUND_SNAP_DISTANCE) {
                    onGround = false;
                }
            } else {
                onGround = false;
            ...
            if (isOnGroundCheck(x, newY, z)) {
                ...
                if (groundY > 0) {
                    ...
                    if (isColliding(x, y, z)) {
                        for (float offset = 0.01f; offset <= 0.5f; offset += 0.01f) {
                            if (!isColliding(x, groundY + offset, z)) {
                                y = groundY + offset;
                                break;
                            }
                        }
    ...
    private boolean isColliding(float px, float py, float pz) {
        ...
        int maxY = (int) Math.floor(py + currentHeight - 0.01f);
        int minZ = (int) Math.floor(pz - halfWidth);
        int maxZ = (int) Math.floor(pz + halfWidth);
    ...
    private boolean isCeilingCollision(float px, float py, float pz) {
        ...
        float halfWidth = PLAYER_WIDTH / 2;
        float currentHeight = getCurrentHeight();
        int checkY = (int) Math.floor(py + currentHeight + 0.01f);
    ...
    public void toggleFlying() {
        if (gameMode.canFly()) {
            ...
            if (flying) {
                velocityY = 0;
                sneaking = false; // Stop sneaking when flying
            }
