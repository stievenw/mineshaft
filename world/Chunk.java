...
public class Chunk {
            ...
            for (int z = 0; z < CHUNK_SIZE; z++) {
                ...
                for (int y = 0; y < CHUNK_HEIGHT; y++) {
                    ...
                    if (y == 0) {
                        ...
                        block = random.nextDouble() > 0.5 ? Blocks.GRAVEL : Blocks.SAND;
                    }
                    else {
    ...
    private Block generateOre(int y, Random random) {
        if (y < 100 && random.nextDouble() < 0.012) return Blocks.COAL_ORE;
        if (y < 64 && random.nextDouble() < 0.008) return Blocks.IRON_ORE;
        if (y < 32 && random.nextDouble() < 0.004) return Blocks.GOLD_ORE;
        if (y < 16 && random.nextDouble() < 0.0015) return Blocks.DIAMOND_ORE;
        return Blocks.STONE;
    }
    ...
    private void generateTree(int x, int y, int z, Random random) {
        ...
        for (int i = 0; i < height; i++) {
            setBlockSafe(x, y + i, z, Blocks.LOG);
        }
            ...
            for (int dy = -2; dy <= 2; dy++) {
                for (int dz = -2; dz <= 2; dz++) {
                    if (Math.abs(dx) + Math.abs(dy) + Math.abs(dz) < 5) {
                        setBlockSafe(x + dx, leafY + dy, z + dz, Blocks.LEAVES);
                    }
                }
            }
        }
